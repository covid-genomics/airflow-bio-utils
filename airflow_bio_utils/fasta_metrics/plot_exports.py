...
def export_figure(result, fs, settings, only_return_binary: bool = False):
    filename, matplot_fig, plotly_fig, df, return_data = (
        None,
        None,
        None,
        None,
    ...
    if len(result) == 5:
        ...
        raise Exception(
            f"Function inside @exported_plot returned tuple of invalid size ({len(result)})."
        )
    if (settings.output_matplot_images and matplot_fig is not None) or (
        settings.create_pdf and only_return_binary and plotly_fig is None
    ):
        bytes_buf = BytesIO()
    ...
    if settings.output_csv and (df is not None):
        ...
        bytes_buf.close()
    if (
        (settings.create_pdf and only_return_binary)
        or settings.output_matplot_images
    ) and not settings.output_matplot_images:
