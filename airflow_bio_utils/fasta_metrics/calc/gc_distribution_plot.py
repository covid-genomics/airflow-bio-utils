...
def gc_distribution_plot(
    ...
    m = int(
        sum([k * v for k, v in zip(counts.keys(), counts.values())])
        / sum(counts.values())
    )
    variances = [(k - m) ** 2 for k in counts.keys()]
    variance = int(
        sum([k * v for k, v in zip(variances, counts.values())])
        / sum(counts.values())
    )
    sigma = math.sqrt(variance)
    x = np.linspace(0, 100, 100)
    gc_content, samples_count = zip(
    ...
    if should_create_matplot(settings):
        ...
        axes.scatter(
            gc_content, samples_count, color=(0.1, 0.6, 0.8), marker="o"
        )
    ...
    if should_create_plotly(settings):
        ...
        df = pd.DataFrame.from_dict(
            dict(
                samples_count=[
                    (
                        samples_count[gc_content.index(val)]
                        if val in gc_content
                        else 0
                    )
                    for val in xint
                ],
                ...
                gc_content_ref=[
                    val * samples_sum for val in gc_content_ref[1:]
                ],
            )
        )
